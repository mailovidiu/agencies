...
class OpenAIConfig {
  ...
  static const String endpoint = String.fromEnvironment('OPENAI_PROXY_ENDPOINT',
      defaultValue:
          'https://us-central1-u-s-departments-and-age-gnkn5k.cloudfunctions.net/dreamopenaiproxy');
...
class OpenAIService {
  ...
  static Future<String> generateDepartmentSummary(
      Map<String, dynamic> departmentData) async {
    final messages = [
      {
        'role': 'system',
        'content':
            '''You are a helpful assistant that creates concise, informative summaries of U.S. government departments and agencies. 
  ...
  static Future<String> compareDepartments(
      List<Map<String, dynamic>> departments) async {
    final departmentInfo = departments
        .map((dept) => {
              'name': dept['name'],
    ...
    final messages = [
      {
        'role': 'system',
        'content':
            '''You are a helpful assistant that compares U.S. government departments and agencies. 
      ...
      },
      {
        'role': 'user',
        'content':
            '''Please compare these government departments/agencies and highlight their key differences and similarities:
    ...
    final messages = [
      {
        'role': 'system',
        'content':
            '''You are a knowledgeable assistant specializing in U.S. government departments and agencies.
      ...
      },
      {
        'role': 'user',
        'content':
            '''Based on this department information, please answer the following question:
    ...
    final messages = [
      {
        'role': 'system',
        'content':
            '''You are a helpful assistant specializing in U.S. government departments, agencies, and public services.
  ...
  static Future<Map<String, dynamic>> routeTaskToDepartment({
    required String task,
    required List<Map<String, dynamic>> candidateDepartments,
  }) async {
    final messages = [
      {
        'role': 'system',
        'content': '''You are a US government service routing assistant.
Return JSON only using this schema:
{
  "primaryId": "string or null",
  "secondaryIds": ["string"],
  "reason": "short explanation",
  "nextSteps": ["up to 3 short action steps"],
  "clarifyingQuestion": "optional short question or null"
}

Rules:
- Choose the single best office for the task when possible.
- Use only candidate IDs that were provided.
- Include at most 2 secondary IDs and 3 next steps.
- If the query is unrelated to government services or none of the candidates fit, set "primaryId" to null and provide a helpful "reason".
- Ask a clarifying question only when confidence is low or the task is ambiguous.'''
      },
      {
        'role': 'user',
        'content': jsonEncode({
          'task': task,
          'candidates': candidateDepartments,
        }),
      }
    ];

    final rawResponse = await chatCompletion(
      messages: messages,
      model: OpenAIConfig.gpt4oMiniModel,
      temperature: 0.3,
      maxTokens: 400,
    ...
    if (decoded is! Map<String, dynamic>) {
      throw Exception('Unexpected task routing response format');
    }
